...
import { FiSend, FiCode, FiTrash2 } from 'react-icons/fi';
import ReactMarkdown from 'react-markdown';
// --- FIX: Import math plugins ---
...
export default function ChatbotPanel({ forwardRef, userId, deckName, fullDeck, currentCardIndex, onDevOpen, onClearChat }) {
    ...
    <div className="bg-card/80 rounded-xl h-full flex flex-col p-4 gap-4">
      <div className="flex-grow border-b border-muted overflow-y-auto p-2 space-y-4 custom-scrollbar min-h-0">
       ...
       <div className="flex-shrink-0 flex justify-between items-center h-5 text-xs text-muted-foreground px-1">
        <div>{lastAiMessage?.tokenInfo && (<p>Tokens: {lastAiMessage.tokenInfo.total_tokens}</p>)}</div>
      ...
      <form onSubmit={handleSubmit} className="flex-shrink-0 flex gap-2">
        <input ref={forwardRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about this card... (/)" className="input-style flex-grow !h-12" disabled={isLoading}/>
        <button type="submit" disabled={isLoading || !input.trim()} className="bg-primary h-12 w-12 flex items-center justify-center rounded-lg text-primary-foreground disabled:opacity-50 disabled:cursor-not-allowed">
